...
def main():
    ...
    if args.sfs:
        ...
        for p in param["name_pop"]:
            ...
            with open(param["out_dir_sfs"]+"SFS_"+p+"_"+param["GR"] + ".fs", 'w') as sfs_out:
    ...
    if args.dadi:
        # added
        print( "Recognize it is a dadi instruction" )
        ...
        for p in param["name_pop"]:
            # added to check if the call was made 
            # raise ValueError("Appel run dadi") # did raise the error
            run_dadi_cli(popid = p, out_dir = param["out_dir_dadi"],
                         sfs_path = param["out_dir_sfs"]+"SFS_"+p+"_"+param["GR"]+".fs",
                         p0 = eval(param["p0"]),
