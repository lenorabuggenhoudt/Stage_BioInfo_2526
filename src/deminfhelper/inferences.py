...
def run_dadi_cli(popid, out_dir, sfs_path, optimizations=None,
    ...
    """
    # added
    print("-------------------------------------------------------- Passe dans run dadi")
    #raise ValueError("Est dans run dadi") # used to check if the call was made
    ...
    if optimizations == None:
        ...
        cmd1 = "".join([
        "dadi-cli InferDM --fs ", sfs_path,
        " --model growth --lbounds ", " ".join(map(str, lower_bounds)),
        " --ubounds ", " ".join(map(str, upper_bounds)),
        " --p0 ", " ".join(map(str, p0)),
        " --output ", out_dir + str(popid),
        " --force-convergence ", str(check_convervence_step)
        ])

        cmd1_bneck = "".join([
        "dadi-cli InferDM --fs ", sfs_path,
        " --model bottleneck --lbounds ", "0.001, 0.001, 1, 1",
        ...
        cmd1_bneck = "".join(["dadi-cli InferDM --fs ", sfs_path, 
                        " --model bottleneck_1pop --lbounds ", "0.001, 0.001, 1, 1",
                        " --ubounds ", "100, 100, 100, 100",
                        " --p0 ", " ".join(map(str, [1,1,1,1])), " --output ", 
                        out_dir+str(popid),  
                        " --optimizations ", str(optimizations), 
                        # check for convergence every 10 optimizations
                        " --check-convergence ", str(check_convervence_step), 
...
def run_stairwayplot2(popid, out_dir, path_to_stairwayplot2):
    ...
    """
    cmd1_originel = "".join(["java -cp ", path_to_stairwayplot2, " Stairbuilder ", out_dir, popid, ".blueprint"])
    
    # added debuggage -----
    fichier_blueprint = "".join([out_dir, popid, ".blueprint"])
    cmd1 = "".join(["java -Djava.awt.headless=true -cp ", path_to_stairwayplot2, " Stairbuilder ", out_dir, popid, ".blueprint"])
    # -----
    print("Type de cmd1 : ", type(cmd1))
    print("Cmd1 : ", cmd1)
    output = subprocess.check_output( cmd1, shell=True)  # the error start here
...
def run_msmc2_process(args):
    ...
    # ADDED --------------------------------------------------------------------------------------------------------------------------------------------
    #print('CONTIG : ', contig) #(1 ou 2)
    #print('INDIVIDUAL : ', individual) #(tsk_0, tsk_1, ...) que ceux qui ont au moins un 1 dans la colonne
    # check

    # step 1 : selectionne certaines lignes du vcf (renvoie un mini vcf)
    cmd_test_step1 = " ".join(["bcftools view -s", individual, "-t", contig, vcf, f" > test_output_run_msmc_step1_{individual}_{contig}"])
    #print("Cmd S1 : ", cmd_test_step1)
    #subprocess.run(cmd_test_step1, shell=True, check=True)


    # step 2 : -> renvoie 1000 tt le temps (comme DP=1000 partout)
    cmd_test_step2 = " ".join(["bcftools view -s", individual, "-t", contig, vcf,
                                "| bcftools query - -f '%INFO/DP\n' | awk '{ sum += $1 } END { print sum/NR }'", f" > test_output_run_msmc_step2_{individual}_{contig}"])
    #print("Cmd S2 : ", cmd_test_step2)
    #subprocess.run(cmd_test_step2, shell=True, check=True)

    # step 3 :
    # a) 
    cmd_test_step3 = " ".join(["bcftools view -s", individual, "-t", contig, vcf,
                    "| bcftools query - -f '%INFO/DP\n' | awk '{ sum += $1 } END { print sum/NR }' |",
                    "{ read value && minDP=$(echo \"$value / 2\" | bc) && maxDP=$(echo \"$value * 2\" | bc);};",
                    "bcftools view -g miss -t", contig, "-s", individual, vcf, f" > test_output_run_msmc_step3_miss_{individual}_{contig}"])
    #print("Cmd S3 : ", cmd_test_step3)
    #subprocess.run(cmd_test_step3, shell=True, check=True)

    # b)
    cmd_test_step3 = " ".join(["bcftools view -s", individual, "-t", contig, vcf,
                    "| bcftools query - -f '%INFO/DP\n' | awk '{ sum += $1 } END { print sum/NR }' |",
                    "{ read value && minDP=$(echo \"$value / 2\" | bc) && maxDP=$(echo \"$value * 2\" | bc);};",
                    "bcftools view -g het -t", contig, "-s", individual, vcf, f" > test_output_run_msmc_step3_het_{individual}_{contig}"])
    ...
    cmd_test_step3 = " ".join(["bcftools view -s", individual, "-t", contig, vcf,
                    "| bcftools query - -f '%INFO/DP\n' | awk '{ sum += $1 } END { print sum/NR }' |",
                    "{ read value && minDP=$(echo \"$value / 2\" | bc) && maxDP=$(echo \"$value * 2\" | bc);};",
                    "bcftools view -g hom -t", contig, "-s", individual, vcf, f" > test_output_run_msmc_step3_hom_{individual}_{contig}"])
    
    #subprocess.run(cmd_test_step3, shell=True, check=True)
    # --------------------------------------------------------------------------- MODIFIED

    cmd_original = " ".join(["bcftools view -s", individual, "-t", contig, vcf,
                    "| bcftools query - -f '%INFO/DP\n' | awk '{ sum += $1 } END { print sum/NR }' |",
                    "{ read value && minDP=$(echo \"$value / 2\" | bc) && maxDP=$(echo \"$value * 2\" | bc);};",
    ...
    cmd = " ".join(["bcftools view -s", individual, "-t", contig, vcf,
                    "| bcftools query - -f '%INFO/DP\n' | awk '{ sum += $1 } END { print sum/NR }' |",
                    "{ read value && minDP=$(echo \"$value / 2\" | bc) && maxDP=$(echo \"$value * 2\" | bc);};",
                    "bcftools view -t", contig, "-s", individual, vcf,
                    "| vcftools --vcf - --minDP $minDP --maxDP $maxDP --recode --stdout | gzip -c >",
                    individual_vcf, ";",
                    "python3", deminfhelper_directory+"/scripts/generate_multihetsep.py",
                    optionnal_kwargs, individual_vcf,
...
def msmc2(contigs, popid, pop_ind, vcf, out_dir, mu, gen_time, kwargs, mask, num_cpus=None):
    ...
    if len(kept_files) == 0:
        raise ValueError("Error! There are no usable file for msmc2, all inputs are empty!")    #here
...
def psmc(ref_genome, contigs, popid, pop_ind, vcf, out_dir, mu, gen_time, kwargs, num_cpus=4):
    ...
    for sample in pop_ind:
        ...
        with open(out_dir+sample+"_consensus.log", 'w') as log:
            ...
            # modified, commentated
            #os.sched_setaffinity(process_pid, cpu_affinity)  # the invalid argument error comes from here 
